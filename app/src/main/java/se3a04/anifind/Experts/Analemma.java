 ...
 * 
 * GPL, MIT License
 * 2012, �an Kafol, Ana Grum
 *
 * This class was found at the following link:
...
public class Analemma {
	 ...
	 * 
	 * @param int the month (0..January, 1..February, ...) for calculating the
	 * sunset
	 * 
	...
	public static double dayLength(double latitude, int year, int month, int day) {
		...
		if (today.after(summerStart) && today.before(autumnStart) || today.before(springStart)
				|| today.after(winterStart)) {
		...
		// Use a negative declination between the summer's and next winter
		// solstice
